        ...
        <div className="min-h-screen bg-black text-white pt-24 px-6 relative print:p-0">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <header className="mb-10 print:hidden">
                    <h2 className="text-4xl font-black text-orange tracking-tight">Meal Plans</h2>
                    <p className="text-gray-400 mt-2 text-lg">Design dietary templates and personalized nutrition guides for your students.</p>
                </header>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 print:hidden">
                        {plans.length > 0 ? (
                                ...
                                <div key={plan._id} className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-8 hover:border-orange/30 transition-all flex flex-col h-full group relative">
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="flex gap-1">
                                            <span className="bg-orange/10 text-orange text-[9px] font-black px-1.5 py-0.5 rounded tracking-tighter uppercase">P: {plan.macros.p}%</span>
                                    ...
                                    <p className="text-gray-400 text-sm mb-6 flex-grow">{plan.desc || "Professional nutritional guide designed for Smart Gym members."}</p>

                                    <div className="space-y-4 mb-8">
                                        {plan.meals.slice(0, 3).map((meal, idx) => (
                                            <div key={idx} className="border-b border-white/5 pb-3">
                                                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">{meal.name}</span>
                                                <span className="text-white text-xs font-semibold leading-tight">{meal.detail}</span>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="col-span-full py-20 text-center bg-white/5 rounded-3xl border border-white/10">
                                <p className="text-gray-500 font-medium italic">No meal plans found in your library.</p>
                            </div>
                        )}
                    </div>
                ) : (
                    <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-8 max-w-4xl mx-auto print:hidden">
                        <div className="mb-8 border-b border-white/10 pb-6">
                            <h3 className="text-xl font-bold text-orange tracking-tight mb-6">
                                {newPlan._id ? "Edit Meal Plan" : "Plan Identity & Macros"}
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                <div className="md:col-span-2">
                                    <label className="text-[10px] text-gray-500 font-black uppercase tracking-widest block mb-2">Meal Plan Title</label>
                                    <input
                                ...
                                <div>
                                    <label className="text-[10px] text-orange font-black uppercase tracking-widest block mb-2">Protein %</label>
                                    <input
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white text-center focus:border-orange/50 outline-none transition-all"
                                        placeholder="40"
                                ...
                                <div>
                                    ...
                                    <input
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white text-center focus:border-blue-400/50 outline-none transition-all"
                                        placeholder="30"
                        ...
                        <div className="mb-8">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-orange tracking-tight">Structured Meals</h3>
                                <button
                            ...
                            <div className="space-y-4">
                                {newPlan.meals.map((meal, idx) => (
                                    <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-black/20 p-5 rounded-xl border border-white/5">
                                        <div className="md:col-span-3">
                                            <label className="text-[10px] text-gray-400 mb-1 block">Meal Name</label>
                                            <input
                                                className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-bold"
                                        ...
                                        <div className="md:col-span-7">
                                            <label className="text-[10px] text-gray-400 mb-1 block">Recommended Foods / Quantities</label>
                                            <textarea
                    ...
                    <div className="hidden print:block text-black bg-white p-12 h-screen overflow-hidden font-serif">
                        ...
                        <div className="flex justify-between items-start border-b-8 border-black pb-8 mb-12">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-xs font-medium text-gray-400 mt-2">{selectedPlan.name}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-0 border-2 border-black mb-12">
                            <div className="border-r-2 border-black p-4">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Issue Date</label>
                                <p className="font-bold text-sm tracking-tight">{new Date().toLocaleDateString()}</p>
                            </div>
                            <div className="border-r-2 border-black p-4">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1 text-orange-600">Protein Target</label>
                                <p className="font-bold text-sm">{selectedPlan.macros.p}</p>
                            </div>
                            <div className="p-4 bg-gray-50">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1 text-blue-600">Carb Target</label>
                            ...
                            {selectedPlan.meals.map((meal, i) => (
                                <div key={i} className="flex gap-12 border-b-2 border-black/5 pb-8 last:border-none">
                                    <div className="w-48 shrink-0">
                                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-gray-400 mb-2">Time</h3>
                                        <p className="text-2xl font-black uppercase tracking-tighter italic">{meal.name}</p>
                                    </div>
                                    <div className="flex-grow">
                                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-gray-400 mb-2">Meal Details</h3>
            ...
            {showAssignModal && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md p-8 animate-in zoom-in-95 duration-200">
                        <h3 className="text-2xl font-bold text-orange mb-6 tracking-tight">Assign Meal Plan</h3>
                        <p className="text-gray-400 mb-6 text-sm italic">Selection: "{selectedPlan?.title}"</p>
